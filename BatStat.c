...
int BLBS_ReadSetting()
{
    ...
    HANDLE hFile;
	uint8_t use_default = FALSE; // If this is true, use default option
	...
	if (0 == GetCurrentDirectoryW(file_size, file_buf))
	{
		free(file_buf);
		JV_ErrorHandle(JVERR_GetCurrentDirectory, TRUE);
	}
	...
	if (use_default)
	{ // Use default option, hFile is invalid at this point
		...
		option.transparency = 255;
        option.height 		= 4;
        option.defaultcolor = RGB(0, 255, 0);
        option.chargecolor 	= RGB(0, 196, 255);
        option.fullcolor	= RGB(0, 255, 255);

        option.color[0]		= RGB(255, 0, 0);
		option.threshold[0]	= 0;
		option.threshold[1] = 20;

		option.color[1]		= RGB(255, 128, 0);
		option.threshold[2]	= 20;
		option.threshold[3] = 50;

		for (uint32_t i = 2; i < BL_COLOR_LEVEL; i++)
		{
			option.color[i]			= 0;
	...
	{ // Read setting from file, hFile is alive at this point
...
{
	if (stat->ACLineStatus == 0) // AC not connected
		return FALSE;
	else if (stat->ACLineStatus == 1) // AC connected, charging
		return TRUE;
...
{
	if (stat->ACLineStatus == 1 && stat->BatteryFlag == 0x08) // AC connected, charging
		return TRUE;
	else if (stat->ACLineStatus == 1 && stat->BatteryFlag != 0x08) // AC connected, not charging
		return FALSE;
